...
export default function HomeContent({
  ...
  useEffect(() => {
    ...
    // Intervalo para atualizar contador (ocupado) e finalizar quando expirar
  ...
  useEffect(() => {
    ...
    channel
      .on("broadcast", { event: "status-change" }, (payload) => {
        const { tarologoId, status, minutos } = payload.payload as {
          tarologoId: string;
          status: "disponivel" | "ocupado" | "indisponivel";
          minutos?: number;
        };
        ...
        setTarologos((prev) =>
          prev.map((t) => {
            ...
            if (status === "ocupado") {
              // Usa os minutos recebidos ou default 30
  ...
  /**
   * Atualiza contador de "ocupado" e muda para indisponível quando expira
   */
  ...
  function mudarStatusTarologo(
    ...
    // Broadcast para outros clientes - inclui os minutos
