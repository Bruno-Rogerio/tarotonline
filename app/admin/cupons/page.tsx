...
type OrigemCupom = "instagram" | "tiktok" | "facebook" | "google" | "influencer" | "indicacao" | null;
...
type Estatisticas = {
  cupons: { ativos: number; inativos: number; expirados: number; total: number };
...
const ORIGENS = [
  { value: "instagram", label: "Instagram", icon: "📸", cor: "from-purple-500 to-pink-500" },
  { value: "tiktok", label: "TikTok", icon: "🎵", cor: "from-gray-800 to-black" },
  { value: "facebook", label: "Facebook", icon: "👍", cor: "from-blue-500 to-blue-600" },
  { value: "google", label: "Google", icon: "🔍", cor: "from-red-500 to-yellow-500" },
  { value: "influencer", label: "Influencer", icon: "⭐", cor: "from-yellow-400 to-orange-500" },
  { value: "indicacao", label: "Indicação", icon: "🤝", cor: "from-green-500 to-emerald-500" },
];
...
export default function AdminCuponsPage() {
  ...
  function abrirModalEditar(cupom: Cupom) {
    ...
    setFormData({
      ...
      desconto_maximo: cupom.desconto_maximo?.toString() || "",
  ...
  async function salvarCupom() {
    ...
    try {
      const payload = {
        ...formData,
        desconto_maximo: formData.desconto_maximo ? parseFloat(formData.desconto_maximo) : null,
        limite_total_usos: formData.limite_total_usos ? parseInt(formData.limite_total_usos) : null,
        data_fim: formData.data_fim || null,
        origem: formData.origem || null,
        influencer_nome: formData.influencer_nome || null,
      };

      const url = cupomEditando ? `/api/cupons/${cupomEditando.id}` : "/api/cupons";
  ...
  async function excluirCupom(cupom: Cupom) {
    if (cupom.total_usos > 0) {
      alert("Não é possível excluir um cupom já utilizado. Desative-o em vez disso.");
      return;
    }
  ...
  function formatarDesconto(tipo: TipoDesconto, valor: number, maximo: number | null) {
    switch (tipo) {
      case "porcentagem":
        let texto = `${valor}% OFF`;
        if (maximo) texto += ` (máx R$${maximo})`;
        return texto;
      case "valor_fixo":
        ...
        <div className="container mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-4">
            <Link href="/admin" className="flex items-center gap-2 group">
              ...
              <span className="text-white/70 group-hover:text-white">Admin</span>
            </Link>
        ...
        {estatisticas && (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
            <div className="bg-gradient-to-br from-purple-500/20 to-purple-600/20 backdrop-blur-sm rounded-2xl p-4 border border-purple-500/30">
              ...
              <div className="text-2xl font-bold text-white">{estatisticas.cupons.total}</div>
            ...
            <div className="bg-gradient-to-br from-green-500/20 to-emerald-500/20 backdrop-blur-sm rounded-2xl p-4 border border-green-500/30">
              ...
              <div className="text-2xl font-bold text-white">{estatisticas.cupons.ativos}</div>
            ...
            <div className="bg-gradient-to-br from-blue-500/20 to-cyan-500/20 backdrop-blur-sm rounded-2xl p-4 border border-blue-500/30">
              <div className="text-3xl mb-2">📊</div>
              <div className="text-2xl font-bold text-white">{estatisticas.usos.total}</div>
            ...
            <div className="bg-gradient-to-br from-yellow-500/20 to-orange-500/20 backdrop-blur-sm rounded-2xl p-4 border border-yellow-500/30">
              <div className="text-3xl mb-2">📅</div>
              <div className="text-2xl font-bold text-white">{estatisticas.usos.hoje}</div>
            ...
            <div className="bg-gradient-to-br from-pink-500/20 to-rose-500/20 backdrop-blur-sm rounded-2xl p-4 border border-pink-500/30">
              <div className="text-3xl mb-2">💰</div>
              <div className="text-2xl font-bold text-white">R$ {estatisticas.descontos.total.toFixed(0)}</div>
            ...
            <div className="bg-gradient-to-br from-indigo-500/20 to-violet-500/20 backdrop-blur-sm rounded-2xl p-4 border border-indigo-500/30">
              <div className="text-3xl mb-2">⏱️</div>
              <div className="text-2xl font-bold text-white">{estatisticas.descontos.minutosExtras}</div>
        ...
        <div className="space-y-4">
          {cupons.length === 0 ? (
            <div className="bg-white/5 backdrop-blur-sm rounded-2xl p-12 text-center border border-white/10">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Nenhum cupom encontrado</h3>
              <p className="text-white/60 mb-6">Crie seu primeiro cupom para começar!</p>
              <button
                onClick={abrirModalNovo}
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`px-3 py-1 rounded-lg text-sm font-bold border ${getCorStatus(cupom.status)}`}>
                    ...
                    <div className="text-2xl font-bold text-white mb-1">
                      {formatarDesconto(cupom.tipo_desconto, cupom.valor_desconto, cupom.desconto_maximo)}
                    </div>
                    
                    {cupom.descricao && (
                      <p className="text-white/60 text-sm mb-2">{cupom.descricao}</p>
                    )}
                    ...
                    <div className="flex flex-wrap gap-2 text-xs text-white/50">
                      ...
                      {cupom.limite_total_usos && (
                        <span>• Limite: {cupom.total_usos}/{cupom.limite_total_usos}</span>
                      )}
                  ...
                  <div className="flex items-center gap-6 text-center">
                    <div>
                      <div className="text-2xl font-bold text-white">{cupom.total_usos}</div>
          ...
          <div className="bg-gradient-to-br from-purple-900 to-indigo-900 rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-white/20">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/80 text-sm mb-1">Código do Cupom *</label>
                  <input
                    type="text"
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, status: e.target.value as StatusCupom })}
              ...
              {/* Descrição */}
              <div>
                <label className="block text-white/80 text-sm mb-1">Descrição (opcional)</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/80 text-sm mb-1">Tipo de Desconto *</label>
                  <select
                    value={formData.tipo_desconto}
                    onChange={(e) => setFormData({ ...formData, tipo_desconto: e.target.value as TipoDesconto })}
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:border-purple-500"
                  >
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-1">
                    {formData.tipo_desconto === "porcentagem" ? "Porcentagem *" : 
                     formData.tipo_desconto === "valor_fixo" ? "Valor (R$) *" : "Minutos *"}
                  </label>
                  <input
                    type="number"
                    ...
                    min="0"
                    step={formData.tipo_desconto === "valor_fixo" ? "0.01" : "1"}
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:border-purple-500"
                  />
                </div>
              </div>

              {/* Desconto Máximo (só aparece para porcentagem) */}
              {formData.tipo_desconto === "porcentagem" && (
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: 30 (limita o desconto a R$ 30)"
                    min="0"
                    step="0.01"
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-purple-500"
                  />
                  <p className="text-white/50 text-xs mt-1">
                    Deixe vazio para sem limite. Ex: 50% com máx R$30 = em compra de R$100, desconto é R$30 (não R$50)
                  </p>
                </div>
              )}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
                <div>
                  <label className="block text-white/80 text-sm mb-1">Valor Mínimo (R$)</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-1">Limite Total</label>
                  <input
                    type="number"
                    value={formData.limite_total_usos}
                    onChange={(e) => setFormData({ ...formData, limite_total_usos: e.target.value })}
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-1">Por Usuário</label>
                  <input
                    type="number"
                    value={formData.limite_por_usuario}
                    onChange={(e) => setFormData({ ...formData, limite_por_usuario: parseInt(e.target.value) || 1 })}
                    min="1"
                    className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white focus:outline-none focus:border-purple-500"
                  />
                </div>
              ...
              <label className="flex items-center gap-3 cursor-pointer">
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, apenas_novos_usuarios: e.target.checked })}
                  className="w-5 h-5 rounded bg-white/10 border-white/20 text-purple-600 focus:ring-purple-500"
                />
                <span className="text-white">Apenas para novos usuários (primeira compra)</span>
              </label>

              {/* Validade */}
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/80 text-sm mb-1">Data Início</label>
                  <input
                    type="datetime-local"
                ...
                <div>
                  <label className="block text-white/80 text-sm mb-1">Data Fim (opcional)</label>
                  <input
                    type="datetime-local"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-white/80 text-sm mb-1">Origem (Marketing)</label>
                  <select
                ...
                {formData.origem === "influencer" && (
                  <div>
                    <label className="block text-white/80 text-sm mb-1">Nome do Influencer</label>
                    <input
                      type="text"
                      value={formData.influencer_nome}
                      onChange={(e) => setFormData({ ...formData, influencer_nome: e.target.value })}
                      placeholder="@usuario"
                      className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-white/40 focus:outline-none focus:border-purple-500"
                    />
                  </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-2xl font-bold text-white">
                    {formatarDesconto(
                      ...
                      formData.desconto_maximo ? parseFloat(formData.desconto_maximo) : null
                    )}
                  </span>
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-4">
                ...
                >
                  {salvando ? "Salvando..." : cupomEditando ? "Salvar" : "Criar Cupom"}
                </button>
              </div>
            </div>
          </div>
